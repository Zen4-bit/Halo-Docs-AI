    ...
    <section className="relative py-32 bg-background overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-5xl md:text-7xl font-bold text-text mb-6 leading-tight tracking-tight">
          ...
          <div className="space-y-4">
            {storyCards.map((card, index) => (
              <motion.div
                ...
                className={`group cursor-pointer rounded-[2rem] p-6 transition-all duration-500 border ${activeCard === index
              ...
              >
                <div className="flex items-start space-x-6">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 transition-all duration-500 ${activeCard === index
                  ...
                  <div className="flex-1">
                    <h3 className={`text-xl font-bold mb-2 transition-colors ${activeCard === index ? 'text-text' : 'text-text-secondary'}`}>
                      {card.title}
                    </h3>
              ...
              {storyCards[activeCard] && (
                <>
                  <motion.div
                    ...
                    className="relative z-10 glass-card border border-border rounded-[2.5rem] p-10 shadow-2xl"
                  >
                    {/* Card Content */}
                    <div className="text-center mb-10">
                      <div className={`w-20 h-20 mx-auto rounded-3xl bg-gradient-to-br ${storyCards[activeCard].color} flex items-center justify-center shadow-lg mb-6`}>
                        <div className="text-white">
                          {storyCards[activeCard].icon}
                        </div>
                      </div>
                      <h3 className="text-3xl font-bold text-text mb-4">{storyCards[activeCard].title}</h3>
                      <p className="text-text-secondary text-lg leading-relaxed">{storyCards[activeCard].story}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 border-t border-border pt-8">
                      {[
                        ...
                        <div key={i} className="text-center">
                          <div className="w-10 h-10 bg-surface-highlight rounded-xl flex items-center justify-center mx-auto mb-3 text-text-secondary">
                            <stat.icon className="w-5 h-5" />
                          </div>
                          <div className="text-text font-bold text-lg">{stat.value}</div>
        ...
        >
          <Link href="/dashboard">
            <button className="btn btn-primary rounded-2xl px-8 py-4 text-lg shadow-xl hover:shadow-2xl">
