          ...
          {categories.map((category) => {
            ...
            return (
              <button
                ...
                className={`group relative flex items-center gap-2.5 rounded-2xl px-5 py-2.5 text-sm font-medium transition-all duration-300 whitespace-nowrap ${
                  isActive
                    ? 'bg-gradient-to-r from-brand-500 to-brand-600 text-white shadow-lg shadow-brand-500/25'
                    : 'bg-slate-100/80 dark:bg-white/[0.08] text-slate-600 dark:text-white/70 hover:bg-slate-200/80 dark:hover:bg-white/[0.12] border border-transparent hover:border-slate-200 dark:hover:border-white/10'
                }`}
              >
                <Icon className={`h-4 w-4 transition-transform duration-300 ${isActive ? '' : 'group-hover:scale-110'}`} />
                {config.label}
                {isActive && (
                  <span className="absolute inset-0 rounded-2xl ring-2 ring-brand-400/30 animate-pulse" />
                )}
              </button>
            );
          })}
