...
export default function CropJPGPage() {
  ...
  const handleCrop = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/tools/crop-jpg', {
        method: 'POST',
        body: formData,
      });
      ...
      setResult({ url, name: file.name.replace(/\.(jpg|jpeg)$/i, '-cropped.jpg'), size: blob.size });
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
      <div className="relative overflow-hidden border-b border-zinc-800">
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-16">
          ...
          <h1 className="text-4xl font-bold text-white mb-3">Crop JPG</h1>
          <p className="text-lg text-zinc-400 max-w-2xl">
            Crop JPEG images with adjustable quality settings.
          </p>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        {!result ? (
          <div className="space-y-8">
            {!file ? (
              ...
              <>
                <div className="p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800">
                  ...
                  <div ref={containerRef} className="relative aspect-video rounded-xl overflow-hidden bg-zinc-800 flex items-center justify-center">
                    {preview && <img src={preview} alt="Preview" className="max-w-full max-h-full object-contain" />}
                    <div className="absolute inset-0 bg-black/50">
                      <div className="absolute border-2 border-rose-400 bg-transparent shadow-lg" style={{ left: cropArea.x, top: cropArea.y, width: cropArea.width, height: cropArea.height }}>
                ...
                <div className="p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800">
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {ASPECT_RATIOS.map(ratio => (
                      <button key={ratio.id} onClick={() => { setAspectRatio(ratio.id); if (ratio.ratio) setCropArea(prev => ({ ...prev, height: prev.width / ratio.ratio! })); }} className={`p-3 rounded-xl border text-center transition-all ${aspectRatio === ratio.id ? 'bg-rose-500/20 border-rose-500/50 text-white' : 'bg-zinc-800/50 border-zinc-700/50 text-zinc-400 hover:border-zinc-600'}`}>
            ...
            {error && <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center gap-3 text-red-400"><AlertCircle className="w-5 h-5 flex-shrink-0" /><p>{error}</p></div>}
            ...
            {file && (
              <button onClick={handleCrop} disabled={processing} className={`w-full py-4 px-6 rounded-xl font-semibold text-lg transition-all flex items-center justify-center gap-3 ${processing ? 'bg-zinc-800 text-zinc-500 cursor-not-allowed' : 'bg-gradient-to-r from-rose-500 to-orange-500 text-white hover:shadow-lg hover:shadow-rose-500/25 hover:scale-[1.02] active:scale-[0.98]'}`}>
                {processing ? <><Loader2 className="w-6 h-6 animate-spin" />Cropping... {progress}%</> : <><Crop className="w-6 h-6" />Crop JPG</>}
              </button>
            )}
          ...
          <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button onClick={handleDownload} className="px-8 py-4 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 text-white font-semibold hover:shadow-lg hover:shadow-green-500/25 transition-all inline-flex items-center justify-center gap-2"><Download className="w-5 h-5" />Download Cropped JPG</button>
              <button onClick={reset} className="px-8 py-4 rounded-xl bg-zinc-800 text-zinc-300 font-semibold hover:bg-zinc-700 transition-colors">Crop Another JPG</button>
            </div>
          </div>
        )}
      </main>
