...
import { Download, CheckCircle2, Crop, Ratio, Image, HardDrive, RotateCw, FlipHorizontal, Sparkles, Wand2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import ToolWorkspaceLayout, { SettingsSection, SettingsSlider, SettingsButtonGroup, SettingsToggle } from '@/components/tools/ToolWorkspaceLayout';
...
export default function CropJPGPage() {
  ...
  // Crop Options
  const [autoSubjectCrop, setAutoSubjectCrop] = useState(false);
  // Transform
  ...
  const handleCrop = useCallback(async () => {
    ...
    try {
      ...
      console.log('Crop JPG Settings:', {
        cropArea, quality, aspectRatio, autoSubjectCrop,
        rotation, flipH, flipV, zoom, autoEnhance, removeExif
      });
      ...
      // Create canvas for cropping and transformations
      ...
      setResult({ 
        ...
        name: file.name.replace(/\.(jpg|jpeg)$/i, '-cropped.jpg'), 
        size: blob.size 
      });
    } catch (err: any) { 
      console.error('Crop error:', err);
      setError(err.message || 'Failed to crop JPG'); 
    }
    finally { setProcessing(false); }
  }, [file, cropArea, quality, aspectRatio, autoSubjectCrop, rotation, flipH, flipV, zoom, autoEnhance, removeExif]);
  ...
  const settingsPanel = (
    <>
      ...
      <SettingsSection title="Crop Options" icon={<Wand2 className="w-4 h-4" />}>
        <SettingsToggle label="Auto subject crop" description="Detect & crop subject" checked={autoSubjectCrop} onChange={setAutoSubjectCrop} />
      </SettingsSection>
      ...
      <SettingsSection title="Quality" icon={<Sparkles className="w-4 h-4" />}>
        ...
        <div className="mt-2"><SettingsToggle label="Auto enhance" checked={autoEnhance} onChange={setAutoEnhance} /></div>
        <div className="mt-2"><SettingsToggle label="Remove EXIF" description="Strip metadata" checked={removeExif} onChange={setRemoveExif} /></div>
      </SettingsSection>
    </>
  );

  return (
    <ToolWorkspaceLayout toolName="Crop JPG" toolIcon={<Crop className="w-5 h-5 text-white" />} toolColor="from-rose-500 to-orange-500" settingsPanel={settingsPanel}
      actionButton={{ label: 'Crop JPG', onClick: handleCrop, disabled: !file, loading: processing, loadingText: `Cropping... ${progress}%`, icon: <Crop className="w-5 h-5" /> }}>
      {!result ? (
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="p-6 rounded-xl bg-white/5 border border-white/10">
              ...
              <div className="relative aspect-video rounded-xl overflow-hidden bg-black/20 flex items-center justify-center">
                ...
                <div className="absolute inset-0 bg-black/50">
                  <div className="absolute border-2 border-rose-400 bg-transparent" style={{ left: cropArea.x, top: cropArea.y, width: cropArea.width, height: cropArea.height }}>
          ...
          {processing && <div className="space-y-2"><div className="w-full h-2 bg-white/10 rounded-full overflow-hidden"><motion.div className="h-full bg-gradient-to-r from-rose-500 to-orange-500" initial={{ width: 0 }} animate={{ width: `${progress}%` }} /></div><p className="text-xs text-white/40 text-center">Cropping...</p></div>}
        </div>
      ) : (
        <div className="max-w-2xl mx-auto space-y-6">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-center py-8">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">JPG Cropped!</h2>
