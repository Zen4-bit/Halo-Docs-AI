...
import { Download, CheckCircle2, Image, Maximize2, Link2, Link2Off, Palette, HardDrive } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import ToolWorkspaceLayout, { SettingsSection, SettingsToggle, SettingsInput } from '@/components/tools/ToolWorkspaceLayout';
...
export default function ResizePNGPage() {
  ...
  const handleResize = useCallback(async () => {
    ...
    try {
      console.log('Resize PNG Settings:', { width, height, preserveTransparency, lockAspectRatio, originalDimensions });
      ...
      setProgress(100);
      setResult({ url: URL.createObjectURL(blob), name: file.name.replace('.png', '-resized.png'), size: blob.size });
    } catch (err: any) { 
      console.error('Resize error:', err);
      setError(err.message || 'Failed to resize PNG'); 
    }
    finally { setProcessing(false); }
  }, [file, width, height, preserveTransparency, lockAspectRatio, originalDimensions]);
    ...
    <>
      <SettingsSection title="Dimensions" icon={<Maximize2 className="w-4 h-4" />}>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setLockAspectRatio(!lockAspectRatio)} className={`mt-5 p-2 rounded-lg ${lockAspectRatio ? 'text-cyan-400 bg-cyan-500/20' : 'text-white/30 bg-white/5'}`}>
  ...
  return (
    <ToolWorkspaceLayout toolName="Resize PNG" toolIcon={<Maximize2 className="w-5 h-5 text-white" />} toolColor="from-cyan-500 to-blue-500" settingsPanel={settingsPanel}
      actionButton={{ label: 'Resize PNG', onClick: handleResize, disabled: !file || width <= 0 || height <= 0, loading: processing, loadingText: `Resizing... ${progress}%`, icon: <Maximize2 className="w-5 h-5" /> }}>
      {!result ? (
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {file && preview && (
            <div className="p-6 rounded-xl bg-white/5 border border-white/10">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                ...
                <div className="p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/20"><p className="text-xs text-cyan-400/60 mb-1">New size</p><p className="text-sm font-medium text-cyan-400">{width} × {height}px</p></div>
              </div>
            </div>
          )}
          ...
          {processing && <div className="space-y-2"><div className="w-full h-2 bg-white/10 rounded-full overflow-hidden"><motion.div className="h-full bg-gradient-to-r from-cyan-500 to-blue-500" initial={{ width: 0 }} animate={{ width: `${progress}%` }} /></div><p className="text-xs text-white/40 text-center">Resizing...</p></div>}
        </div>
      ) : (
