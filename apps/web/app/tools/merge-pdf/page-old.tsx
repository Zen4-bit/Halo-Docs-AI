    ...
    <div
      ...
      className={`group flex items-center gap-4 p-4 rounded-xl border transition-all duration-200 ${
        isDragging 
          ...
          : 'tool-bg-surface tool-border hover:tool-bg-highlight'
      }`}
    >
      <button
        {...attributes}
        {...listeners}
        className="p-2 rounded-lg hover:tool-bg-highlight cursor-grab active:cursor-grabbing tool-text-muted hover:text-amber-400 transition-colors"
      >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium tool-text truncate">{pdf.name}</p>
        <p className="text-sm tool-text-muted">{formatSize(pdf.size)}</p>
      </div>
      
      <button
        onClick={() => onRemove(pdf.id)}
        className="p-2 rounded-lg tool-text-muted hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100"
      >
...
export default function MergePDFPage() {
  ...
  return (
    <div className="tool-page min-h-[calc(100vh-5rem)]">
        ...
        {!result ? (
          <div className="space-y-8">
            ...
            <div
              ...
              className="tool-upload-zone group hover:border-red-500/50 hover:bg-red-500/5"
            >
              <input
                type="file"
                accept=".pdf"
              ...
              <div className="text-center">
                <div className="inline-flex p-4 rounded-2xl tool-bg-highlight group-hover:bg-red-500/20 transition-colors mb-4">
                  <Upload className="w-10 h-10 tool-text-muted group-hover:text-red-400 transition-colors" />
                </div>
                <h3 className="text-xl font-semibold tool-text mb-2">Drop PDF files here</h3>
                <p className="tool-text-muted mb-4">or click to browse • PDF files only</p>
            ...
            {pdfs.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => setPdfs([])}
                    className="text-sm tool-text-muted hover:text-red-400 transition-colors"
                  >
                    Clear all
                  </button>
                </div>
                ...
                <p className="text-sm tool-text-muted text-center">
            ...
            {/* Options */}
            {pdfs.length > 0 && (
              <div className="p-6 rounded-2xl tool-bg-surface tool-border border">
                <h3 className="font-semibold tool-text mb-4 flex items-center gap-2">
                  <Settings2 className="w-5 h-5 text-amber-400" />
                  Merge Options
                </h3>
                <div className="grid sm:grid-cols-2 gap-4">
                  <label className="flex items-center gap-3 p-4 rounded-xl tool-bg-highlight tool-border border cursor-pointer hover:border-amber-500/50 transition-colors">
                    <input
                      type="checkbox"
                      checked={addBookmarks}
                      onChange={(e) => setAddBookmarks(e.target.checked)}
                      className="w-5 h-5 rounded tool-border text-amber-500 focus:ring-amber-500/20"
                    />
                    <div>
                      <div className="flex items-center gap-2 tool-text font-medium">
                        <Bookmark className="w-4 h-4 text-amber-400" />
                        Add Bookmarks
                      </div>
                      <p className="text-sm tool-text-muted">Create bookmarks for each file</p>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-3 p-4 rounded-xl tool-bg-highlight tool-border border cursor-pointer hover:border-amber-500/50 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-sm tool-text-muted">Strip document metadata</p>
                    </div>
                  </label>
                </div>
              </div>
            ...
            <button
              ...
              className={`w-full py-4 px-6 rounded-xl font-semibold text-lg transition-all flex items-center justify-center gap-3 ${
                pdfs.length < 2 || processing
                  ? 'tool-bg-highlight tool-text-muted cursor-not-allowed'
            ...
            {/* Progress */}
            {processing && (
              <div className="w-full h-2 tool-bg-highlight rounded-full overflow-hidden">
                <div
          ...
          <div className="text-center py-12 space-y-6">
            ...
            <h2 className="text-2xl font-bold tool-text">PDFs Merged Successfully!</h2>
            <p className="tool-text-secondary">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
              ...
              <button
                onClick={reset}
                className="px-8 py-4 rounded-xl tool-bg-surface tool-text-secondary font-semibold hover:tool-bg-highlight transition-colors"
              >
                Merge More PDFs
              </button>
            </div>
